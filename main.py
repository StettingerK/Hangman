...
def buchstabe_pruefen(hiden_wort:str, visible_wort, buchstabe:str) -> str:
    for zeichen in range(0,len(hiden_wort)):
        if hiden_wort[zeichen] == buchstabe:
            visible_wort[zeichen] = buchstabe
    return visible_wort
...
def main():
    ...
    visible_wort:str = "".join("_ " for count in range(0,len(hiden_wort))).strip()
    ...
    while game_running:
        ...
        spieler_count = spieler_count +1 if spieler_count < len(spieler) else 0 # spieler_count hochzählen sonst zurücksetzen
        ...
        darstellungsschicht.wort_darstellen(visible_wort)
